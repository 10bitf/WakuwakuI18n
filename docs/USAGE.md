...
| 导出 | 从哪导入 | 签名 | 用途 |
|---|---|---|---|
| `PRESET` | `wakuwaku-i18n/i18next-preset` | ⚠️ **将退役**：i18next 的复数只认魔法变量名 `count`、一条消息只能一个复数、序数要另一套 key 后缀 —— 三条都做不了海外版（`docs/specs/2026-09-09-icu-compile-design.md` §1）。**最后一个消费方迁走就删**，迁到 `/compile` 或 Paraglide 都算。冻结的配置对象 | 三端必须一致的 i18next 解析规则:`nsSeparator:false`、`keySeparator:'.'`、`interpolation` 的单花括号。**不是取词逻辑,是配置常量** |
| `withPreset` | `wakuwaku-i18n/i18next-preset` | ⚠️ **将退役**：i18next 的复数只认魔法变量名 `count`、一条消息只能一个复数、序数要另一套 key 后缀 —— 三条都做不了海外版（`docs/specs/2026-09-09-icu-compile-design.md` §1）。**最后一个消费方迁走就删**，迁到 `/compile` 或 Paraglide 都算。`withPreset(options?) → object` | 把 PRESET 与消费方选项合并后交给 `i18next.init()`。**对 `interpolation` 做合并而非替换** —— 直接展开 PRESET 会丢掉 prefix/suffix,而症状是「占位符不报错、只是原样不替换」 |
...
| `compileTable` | `wakuwaku-i18n/compile` | `compileTable(flatTable, locale, opts?) → string` | 📦 **可选零件**（要自己装 `@messageformat/*`，见 §5.0）。**目标环境没有 `Intl` 时用它** —— 编译产物把复数规则编进函数体，对「有没有 `Intl`」免疫，而运行时方案（含 Paraglide）的答案完全取决于它。把**扁平**表编译成 ES 模块源码（`export default { key: fn }`）。编不过一次报全。🔴 默认拦 `#`（它编成 `number()`，内部是 `Intl.NumberFormat`，而安卓微信没有 `Intl`）——纯 web 环境用 `{ allowIntl: true }` 放行。Node-only |
| `compileTables` | `wakuwaku-i18n/compile` | `compileTables(nestedTables, locale, opts?) → string` | 📦 **可选零件**（要自己装 `@messageformat/*`，见 §5.0）。**目标环境没有 `Intl` 时用它** —— 编译产物把复数规则编进函数体，对「有没有 `Intl`」免疫，而运行时方案（含 Paraglide）的答案完全取决于它。同上，喂几张**嵌套**表（`{ app: {...} }`），命名空间即键名，与 `loadTables` 口径一致。Node-only |
| `flattenTable` | `wakuwaku-i18n/compile` | `flattenTable(node, prefix?, out?) → { [key]: string }` | 嵌套表展平成点分 key。`compileTables` 内部用它，导出是给宿主自建流程用 |
| `makeT` | `wakuwaku-i18n/make-t` | `makeT(table, opts?) → (key, vars?) => string` | 📦 **可选零件**（要自己装 `@messageformat/*`，见 §5.0）。**目标环境没有 `Intl` 时用它** —— 编译产物把复数规则编进函数体，对「有没有 `Intl`」免疫，而运行时方案（含 Paraglide）的答案完全取决于它。取词纯逻辑：喂编译产物回 `t`。兜三件事——**`vars` 缺省给 `{}`**（编译函数拿 `undefined` 会抛，而不带参数的调用到处都是）、缺 key 回空串 + 告警、开发期「占位符没填上」哨兵。**与构建系统无关，所以能被 Node 直测** |
...
**动态拼 key(前缀式使用)**:采到的字面量若不是完整 key,但**名下至少有一个已定义的叶子**,
就算「用了那一族」——不报未定义,名下叶子也不再报没人用。两种写法都认:

```js
t('app.uilab.sc.' + s)                          // 采到 'app.uilab.sc.'(带尾点)
const KNOWS = ['app.scout.wait.k1'];  t(k + '.t')  // 采到 'app.scout.wait.k1'(不带尾点)
```

名下**一个叶子都没有**的仍然报未定义,所以拼错的前缀照样抓得住——这条比不认前缀时更严,
不是更松。两个边界:段必须整段对齐(比 `前缀 + '.'`,`app.scout.wa` 盖不住 `wait` 一族);
带尾点的写法永远当前缀,不许精确命中(`t('a.b.' + x)` 拼不出 `a.b` 自己,所以 `a.b`
若无人以完整形式引用,照报没人用)。
