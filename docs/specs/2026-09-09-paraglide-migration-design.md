# Paraglide 迁移设计（🛑 **已停，只留 WakuwakuDark**）

> ## 🛑 状态：这份不是执行文档
>
> **2026-09-09 傍晚三份独立审查把这个计划拦下了。** 结论与全部读数在
> [`2026-09-09-三份审查与现状.md`](./2026-09-09-三份审查与现状.md) —— **先读那份**。
>
> 两句话说清为什么停：
>
> 1. **迁移的决定性理由不成立。** 翻译工具链（Fink / Sherlock）读的是 **inlang 工程**
>    （11 行配置），Paraglide 只是另一个消费它的编译器。已实测：只装 `@inlang/sdk`、
>    没有任何 Paraglide 的目录，读得到 14 条 key、改一条还能写回 JSON。
> 2. **Racing 上买到的是零。** 拿 Racing 自己的 rollup 实测：字面量索引能摇干净（10/917），
>    而命名空间挂 `globalProperties` 完全摇不掉（917/917）—— 而 Racing 94 个 `.vue`
>    全是 Options API，只能走后者。
>
> **现在成立的是**：不是「换不换引擎」，是「哪个项目适合哪个引擎」。
> WakuwakuDark（Astro + TS）保留 Paraglide 并已验收；Racing / Photoman（uni-app +
> Options API + 无 TS）不迁。
>
> 下面的内容作为**调研与实测记录**仍然有效（§3 那 11 条读数、§12 的对照），
> 但 §6 / §7 的做法已被 §14 改判，**别照着做**。
...
> 🔴 **本节被 §14.1 改判。** 下面写的「合并成单文件」是错的 —— `pathPattern` 吃数组，
> 所以**目录、文件、命名空间全都留着**，只改文件内部的嵌套形状。照 §14.1 做。

实测 ⑧：`plugin-icu1` 只吃**扁平**的 JSON（嵌套的编出 0 条）。
...
> 🔴 **本节被 §14.2 改判。** 标识符形态（`m.app_nav_data()`）**根本不存在** ——
> key 里带点号时 Paraglide 只生成字符串名导出。正确形态是 `m['app.nav.data']()`，
> 而它 key 原样保留、codemod 更机械、类型检查与补全也都还在。照 §14.2 做。
