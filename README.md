...
Wakuwaku 项目共用的**文案管控工具**。核心是两件市面上没人替我们做的事:

- **禁止源码里写裸中文**(`lint-raw`,开源界没有替代品;注意它守的是**源语言**,见 USAGE 第 6 节的⚠️块)
- **语言包的完整性**(`check`:用了没定义的 key / **跨语言缺失** / 占位符不一致 /
  `assets` 声明的非文本交付物有没有兑现)

外加一件工具(`load` 读文案表),以及**按端可选**的取词引擎 —— 见下。

> ## 这个库是什么
>
> **文案的校验层。取词是可选零件。**
>
> 校验那部分（`lint-raw` 裸中文硬卡、`check` 跨语种完整性）**跟用哪个取词引擎无关** ——
> 2026 年在引擎上换过三次方向，这两件一行没改过。它们也是市面上没人替我们做的那两件：
> `lint-raw` 无开源替代；`check` 那条「中文有、英文没有就红」，Paraglide 是**静默回退**的。
>
> 取词引擎**按端选**，装了才有、不装一个字节都不下（`docs/USAGE.md` §5.0）：
>
> | 端 | 选什么 | 为什么 |
> |---|---|---|
> | 微信小程序（**没有 `Intl`**） | `/compile` + `/make-t` | 编译产物把复数规则编进函数体，对「有没有 `Intl`」免疫 |
> | 浏览器（Astro 那几个） | Paraglide（外部包） | 有 `Intl`，且能拿到类型安全与 tree-shaking |
> | 还没迁的 | `/i18next-preset` ⚠️ **将退役** | 它的复数模型做不了海外版；最后一个消费方迁走就删 |
>
> 翻译工具链（Fink / Sherlock）读的是 **inlang 工程**（11 行配置），**跟选哪个引擎无关** ——
> 这一条 2026-09-09 搞错过一次，据此推出了一轮不必要的迁移，记在
> [`docs/specs/2026-09-09-三份审查与现状.md`](docs/specs/2026-09-09-三份审查与现状.md)。
